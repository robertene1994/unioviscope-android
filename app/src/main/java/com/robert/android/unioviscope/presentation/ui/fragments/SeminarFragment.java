 ...
 */
public class SeminarFragment extends AbstractFragment implements SessionPresenter.View {

    // vistas que se ocultan al realizar alguna operación (cargando)
    private ListView mSeminarSessionsListView;

    private View mSeminarLayout;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mSeminarLayout = inflater.inflate(R.layout.fragment_seminar, container, false);
        mSeminarSessionsListView = mSeminarLayout.findViewById(R.id.seminarSessionsListView);

        View mProgressBarLayout = mSeminarLayout.findViewById(R.id.progressBarConstraintLayout);
        setProgressBarLayout(mProgressBarLayout);
        addViewsToHide(mSeminarSessionsListView);

        mSessionPresenter.getSessions(mSubject, GroupType.SEMINAR);
        return mSeminarLayout;
    }
    ...
    public void showSessions(ArrayAdapter<Session> adapter) {
        mSeminarSessionsListView.setAdapter(adapter);
    }

    @Override
    public void onSessionsEmpty() {
        if (this.getUserVisibleHint())
            SnackbarUtil.makeLong(mSeminarLayout, R.string.msg_no_sheduled_seminar_sessions);
    }

    @Override
    public void noInternetConnection() {
        SnackbarUtil.makeLong(mSeminarLayout, R.string.msg_no_internet_connection);
    }

    @Override
    public void serviceNotAvailable() {
        SnackbarUtil.makeLong(mSeminarLayout, R.string.msg_service_not_avabile);
    }
}
